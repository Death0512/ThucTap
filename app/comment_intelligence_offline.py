...
BATCH_SIZE     = 3
DELAY          = 0.5   # llm_client enforces 40 req/min; this is just loop breathing room
...
def check_llm():
    if llm_client.check_llm():
        print(f"   LLM API connected — model: {llm_client.LLM_MODEL}")
        return True
    print(f"   LLM API not reachable")
    return False
...
def _llm_analyze(prompt):
    raw = llm_client.analyze(prompt, temperature=0.1)
    if raw:
        return extract_json_from_response(raw)
    return None


def analyze_batch_llm(comments_batch):
    ...
    results = []
    for i, c in enumerate(comments_batch):
        single_prompt = f"""Analyze this Facebook comment and return ONLY a JSON object.
...
{{"sentiment": "positive|negative|neutral", "emotion": "support|anger|sarcasm|aggressive", "stance": "support_post|oppose_post|neutral_discussion", "language": "English"}}"""
        ...
        single_result = _llm_analyze(single_prompt)
        if isinstance(single_result, list) and single_result:
            single_result = single_result[0]
        if isinstance(single_result, dict):
            results.append({
                "index": i + 1,
                "sentiment": single_result.get("sentiment", "neutral"),
                "emotion":   single_result.get("emotion",   "support"),
                "stance":    single_result.get("stance",    "neutral_discussion"),
                "language":  single_result.get("language",  "Unknown"),
            })
        else:
            results.append({
                "index": i + 1,
...
def analyze_comments(db_file, label="socmint", batch_id=None):
    ...
    for i in range(0, total, BATCH_SIZE):
        ...
        results = analyze_batch_llm(batch)
...
def run_all(manual_batch_id=None):
    """Analyze comments from both DBs using LLM."""
    print("\n" + "═"*65)

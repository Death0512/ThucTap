        ...
        with FBSession(cookie_file=COOKIE_FILE, headless=True) as page:
            for section in DIRECTORY_SECTIONS:
                ...
                try:
                    ...
                    responses = _pw.capture_graphql(
                        page, trigger_fn=_nav,
                        filter_fn=lambda b: any(k in str(b) for k in ("field_type", "group_key")),
                        timeout_ms=7000, retries=3,
                    )

                    fields = _pw.extract_about_from_graphql(responses)

...
TEXT_RATIO_THRESHOLD  = 0.75  # ≥ 75% → heavily text, skip vision LLM call
TEXT_MID_THRESHOLD    = 0.50  # 40-74% → OCR + text-aware vision LLM prompt
...
def analyze_image_llm(image_b64, mime_type, prompt):
    try:
        raw = llm_client.analyze_with_image(image_b64, mime_type, prompt, temperature=0.1)
        if not raw:
            return None
        if raw.startswith("```"):
            raw = raw.split("```")[1]
...
def analyze_images(db_file=DB_FILE):
    ...
    for i, photo in enumerate(photos, 1):
        ...
        if text_ratio >= TEXT_RATIO_THRESHOLD:
            # ≥ 75% — heavily text, skip vision LLM call
            print(f"      Heavily text-based — skipping vision analysis")
            result = {
            ...
            # 40-74% — extract OCR text + text-aware vision LLM prompt
        ...
        print(f"      Analyzing with {llm_client.LLM_MODEL}...")
        result = analyze_image_llm(b64, mime, prompt)

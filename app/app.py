...
def system_check():
    ...
    app.config['LLM_OK'] = llm_ok
    app.config['LLM_MODEL'] = llm_client.LLM_MODEL
    ...
    print(f"  Provider : {llm_client.LLM_BASE_URL}")
    print(f"  Cookies  : {' Found' if app.config['COOKIES_OK'] else ' Missing — use Import Cookies tool'}")
    print(f"{'═'*60}\n")
...
def check_cookies_valid():
    """
    Check if fb_cookies.json works on Facebook using Playwright.
    """
    cookie_path = os.path.join(BASE_DIR, 'fb_cookies.json')

    if not os.path.exists(cookie_path):
        return False, 'Cookie file not found — import cookies first'
    if os.path.getsize(cookie_path) < 50:
        return False, 'Cookie file is empty — import cookies first'
    ...
    try:
        ...
        if not cookies:
            return False, 'Cookie file is empty — import cookies'

        # Quick check: must contain c_user
        ...
        if 'Use another profile' in page_source:
            return False, 'Session requires re-authentication — please re-import cookies'

        logged_in_signals = ['c_user', '"USER_ID"', 'id="mount_0_0_']
...
def run_pipeline_auto(profile_url, profile_id, scan_type):
    ...
    model  = app.config.get('LLM_MODEL', llm_client.LLM_MODEL)
...
def run_pipeline_manual(batch_id, label, urls, scan_type='medium'):
    ...
    try:
        ...
        import fb_manual_unified
        # Pass URLs directly — no input() blocking
        fb_manual_unified.main(MAX_URLS=len(urls), urls=urls)
...
def home():
    ...
    system   = {
        'model':       app.config.get('LLM_MODEL', llm_client.LLM_MODEL),
        'llm_ok':      app.config.get('LLM_OK', False),
        'cookies_ok':  app.config.get('COOKIES_OK', False),
        'provider':    llm_client.LLM_BASE_URL,
    }
...
def change_model():
    ...
    app.config['LLM_OK'] = llm_client.check_llm()
    ...
    def _run():
        ...
        try:
            ...
            with open(cookie_path, 'w', encoding='utf-8') as _f:
                _json.dump(pw_cookies, _f, ensure_ascii=False, indent=2)
...
def api_import_cookies():
    ...
    normalise to Playwright format and save as fb_cookies.json.
    """
    import json as _json
    ...
    try:
        ...
        # Normalise to Playwright-compatible JSON format
        # Cookie-Editor fields: name, value, domain, path, expires/expirationDate, httpOnly, secure, sameSite
        converted = []
        for c in cookies:
            pw_cookie = {
                'name':     c.get('name', ''),
            ...
            if expires and isinstance(expires, (int, float)) and expires != -1:
        ...
        # Save to fb_cookies.json (Playwright-native format)
        cookie_path = os.path.join(BASE_DIR, 'fb_cookies.json')
        with open(cookie_path, 'w', encoding='utf-8') as _f:
            _json.dump(converted, _f, ensure_ascii=False, indent=2)

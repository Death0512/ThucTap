    ...
    try:
        ...
        with sync_playwright() as pw:
            ...
            for section in sections:
                ...
                try:
                    gql_responses = _pw.capture_graphql(
                        page,
                        trigger_fn=lambda: (
                        ...
                        filter_fn=lambda b: any(
                            k in str(b) for k in ("field_type", "group_key")
                        ),
                        timeout_ms=6000,
                        retries=2,
                    )

                    gql_fields = _pw.extract_about_from_graphql(gql_responses)
                    if gql_fields:
                        for f in gql_fields:
                            ft = f["field_type"]
                            if ft not in all_fields:
                                all_fields[ft] = f["value"]
                    else:
                        source = page.content()
